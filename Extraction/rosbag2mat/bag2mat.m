function [data, fields, topics] = bag2mat(inp, do_save)
...
if isdir(inp) % directory with bag files
  ...
  for k = 1:length(flst)
    ...
    [data, fields, topics] = extractSingleFile(bagf, matf, do_save);
  end
  ...
  [data, fields, topics] = extractSingleFile(inp, matf, do_save);
end
end


function [data, fields, topics] = extractSingleFile(bagf, matf, do_save)
